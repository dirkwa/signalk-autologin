...
<https://github.com/dirkwa/signalk-autologin>.

Before opening a pull request, run the full local chain:

```bash
npm run format && npm run build && npm run lint && npm test
```
...
it under any terms, including as part of signalk-autologin releases. You confirm
